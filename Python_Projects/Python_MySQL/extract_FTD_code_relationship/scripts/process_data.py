def _read_and_write(raw_file_route, pipe_file_route, pairs):
    ...
    js_file = open(pipe_file_route, 'w')
    ...
    for line in raw_file:
        line = line.strip()
        code = line[pairs[0][0] - 1:pairs[0][1]].strip()
        codes.append(code)
        desc = line[pairs[2][0] - 1:pairs[2][1]].strip()
        desc_dict[code] = desc

    relationship_dict = get_relationship(codes)

    js_file.write('hts_cd = {};\r')

    relationship_keys = list(relationship_dict.keys())
    relationship_keys.sort()
    for key in relationship_keys:
        new_line = 'hts_cd[\"{0}\"] = ['.format(key)
        array = relationship_dict[key]
        size = len(array)
        for index in range(0, size - 1):
            new_line = new_line + '\"' + array[index] + '\", '
        new_line = new_line + '\"' + array[size - 1] + '\"];\r'
        js_file.write(new_line)

    js_file.write('\r')
    js_file.write('hts_desc = {};\r')

    desc_keys = list(desc_dict.keys())
    desc_keys.sort()
    for key in desc_keys:
        new_line = 'hts_desc[\"{0}\"] = \"{1}\";\r'.format(key, desc_dict[key].title())
        js_file.write(new_line)
...
def get_relationship(codes):
    ...
    for code in codes:
        ...
        if len(code) == len(current_parent):
            ...
            stack.pop()
            # since, each time a code without children got popped, the next member in the stack is surely the real
            # parent level of the current entry (code in this case)
            ...
            # in this case, since the current code is longer than the previous one, it means it has to be its child
            real_parent = current_parent
        else:
            # in this case, the current code is shorted than the previous one, this means the previous "parent" has
            # exhausted all its children, and that the current code is its sibling, all previous silbings should
            # be popped out, until it reaches the current code's parent
...
def process_hsdesc(data_folder, output_folder):
    ...
    hsdesc_pipe_route = output_folder + 'HSDESC.js'
    pairs = [
        [1, 6],
        [7, 156],
        [157, 206]
