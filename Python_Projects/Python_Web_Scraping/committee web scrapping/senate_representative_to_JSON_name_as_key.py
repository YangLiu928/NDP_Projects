...
def get_current_senate_representative_data():
    ...
    for row in rows:
        # every other row in the table is a blank row with only an image tage
        # need to detect whether this row contains table or not
        if row.find_all('td')[0].find('a'):
            result = {}
            results[row.find_all('td')[0].find('a').string] = result        
...
if __name__ == '__main__':
    ...
    with open('senate_test_name_as_key.JSON', 'w') as outfile:
