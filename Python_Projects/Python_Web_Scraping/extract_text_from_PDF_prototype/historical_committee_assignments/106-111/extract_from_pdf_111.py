...
if __name__ == '__main__':
    ...
    doc = _convert_pdf_to_txt('input/111.pdf')
    members = []
    assignments = []
    ...
    # members has 440 results, and assignments has 439 results
    # for the case of document 111.pdf, the problem occurs when reaching
    ...
    #     if re.search(''members[index]):
    #         continue
    #     result.append(members[index] + '                     ' + assignments[index])
    ...
    while assignment_index < min_length:
        ...
        if re.search('Herseth Sandlin, Stephanie', member):
            ...
            output['committee_assignments'] = ['Agriculture','Natural Resources', 'Select Global Warming', 'Veterans\' Affairs']
            member_index += 1
            assignment_index += 0
        elif re.search('Higgins, Brian', member):
            output.update(_get_member_data(member))
            output['committee_assignments'] = [_get_committee_assignments(assignment)[-1]]
            member_index += 1
            assignment_index += 1
        else:
            output.update(_get_member_data(member))
    ...
    with open('output/111_adjusted.JSON', 'w') as outfile:
