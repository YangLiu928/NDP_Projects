...
if __name__ == '__main__':
    ...
    doc = _convert_pdf_to_txt('input/109.pdf')
    members = []
    assignments = []
    ...
    # members has 437 results, and assignments has 438 results
    # for the case of document 109.pdf, the problem occurs when reaching
    # member "Everett, Terry, 2nd AL", where the assignments was messed up
    # due to the page break in the PDF document, special treatment needed
    # result = []
    ...
    while assignment_index < min_length:
        ...
        if re.search('Everett, Terry', member):
            output.update(_get_member_data(member))
            output['committee_assignments'] = _get_committee_assignments(assignment) + ['Veterans\' Affairs']
            member_index += 1
            assignment_index += 2
        else:
            output.update(_get_member_data(member))
    ...
    with open('output/109_adjusted.JSON', 'w') as outfile:
